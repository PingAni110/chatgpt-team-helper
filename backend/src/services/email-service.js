...
function buildOpenAccountsSweeperBody(summary) {
  ...
  const htmlParts = [
    `<p>开放账号超员扫描已完成。</p>`,
    `<p>扫描账号数：${scannedCount ?? 0}，阈值：joined &gt; ${maxJoined ?? ''}，本次踢出：${totalKicked ?? 0}，异常：${derivedAbnormalSpaceCount}</p>`,
  ...
  return {
    html: htmlParts.join('\n'),
    text: `开放账号超员扫描已完成。\n扫描账号数：${scannedCount ?? 0}，阈值：${maxJoined ?? ''}，本次踢出：${totalKicked ?? 0}，异常：${derivedAbnormalSpaceCount}${Number(scanCreatedWithinDays) > 0 ? `\n扫描范围：最近 ${scanCreatedWithinDays} 天创建的开放账号` : ''}${textFailures}\n\n${textRows}${textTime}`
  }
}
